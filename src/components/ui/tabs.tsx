...
const Chip = ({ text, selected, setSelected }: ChipProps) => {
  return (
    <button
      ...
      className={`${selected ? 'text-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'} text-sm transition-colors px-4 py-2 rounded-full relative cursor-pointer`}
    >
      <span className="relative z-10">{text}</span>
      {selected && <motion.span layoutId="pill-tab" transition={{ type: 'spring', duration: 0.5 }} className="absolute inset-0 z-0 bg-gradient-to-br from-sky-500 to-blue-500 rounded-full" />}
    </button>
  )
}
