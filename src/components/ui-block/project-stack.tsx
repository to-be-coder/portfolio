...
export function HorizontalStack({ title, children, titleColor = 'text-black', borderColor = 'border-[#000000]', className = '', mobileCols = 1, desktopCols = 3 }: HorizontalStackProps) {
  ...
  return (
    <div className={`${borderColor} flex flex-col ${className}`}>
      {title && <h2 className={`text-2xl mb-2 md:mb-4 ${titleColor}`}>{title}</h2>}
      <div className={`grid ${mobileColClass} ${desktopColClass} gap-4`}>{children}</div>
    </div>
  )
}
